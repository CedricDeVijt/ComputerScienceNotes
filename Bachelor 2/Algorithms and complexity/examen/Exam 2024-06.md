...
**Stelling van Immerman-Szelepcsényi**: De complexiteitsklasse NSPACE(s(n)) is gesloten onder complementatie voor elke ruimteconstructeerbare functie s(n) ≥ log n. Dit betekent dat voor elke niet-deterministische Turingmachine die in ruimte s(n) werkt, er een niet-deterministische Turingmachine bestaat die in dezelfde ruimte het complement van het probleem beslist.

**Uitleg bewijs**:
Gegeven een gerichte graaf $G$, knopen $s$ (begin) en $t$ (eind), en $c$ (aantal knopen bereikbaar vanuit $s$), beslist het bewijs of er géén pad is van $s$ naar $t$. Dit gebeurt via een niet-deterministische algoritme in logaritmische ruimte:

1. **Bereikbaarheidscheck**: Het algoritme gebruikt een niet-deterministische procedure om te verifiëren of een knoop $v$ bereikbaar is vanaf $s$ binnen $c$ stappen. Dit doet het door alle mogelijke paden van lengte ≤ $c$ te raden en te tellen hoeveel knopen (exact $c$) bereikbaar zijn.

2. **Complementatie**: Om te beslissen dat er géén pad van $s$ naar $t$ is, controleert het algoritme of $t$ niet in de set van $c$ bereikbare knopen zit. Dit gebeurt door:

   - Een teller bij te houden voor het aantal knopen dat bereikbaar is vanuit $s$.
   - Voor elke knoop $v \neq t$, niet-deterministisch te verifiëren of $v$ bereikbaar is (via een pad van $s$ naar $v$).
   - Als precies $c$ knopen bereikbaar zijn en $t$ niet wordt bereikt, is er geen pad van $s$ naar $t$.

3. **Ruimte-efficiëntie**: Het algoritme gebruikt slechts logaritmische ruimte, omdat het alleen een constante hoeveelheid variabelen (zoals een teller en knoop-ID’s) opslaat en de graafstructuur direct benadert.

Het bewijs toont aan dat het complement van het bereikbaarheidsprobleem (geen pad van $s$ naar $t$) in dezelfde ruimteklasse beslisbaar is, wat de sluiting onder complementatie bewijst.
