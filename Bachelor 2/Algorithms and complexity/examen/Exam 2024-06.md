...
**Algoritme van Euclides** (zoals beschreven in het document, pagina 14-15):
...
- **Inputgrootte**: De input ⟨x, y⟩ heeft grootte n = O(max{log₂ x, log₂ y}), omdat x en y binair worden voorgesteld.
- **Aantal iteraties**: In elke iteratie (stappen 2 en 3) halveert minstens één van de getallen x of y (zie pagina 15). Dit komt doordat na de modulo-operatie x < y wordt, en na het verwisselen y < x/2 of x < y/2. Het aantal iteraties is dus O(log(max{x, y})) = O(n).
...
- Een taal L ∈ NP als er een deterministische Turingmachine V (de verifiër) bestaat, en een polynomiale functie p, zodat voor elke input x:
  - Als x ∈ L, er een certificaat c bestaat (met lengte |c| ≤ p(|x|)) zodat V(x, c) "aanvaardt" in polynomiale tijd.
  - Als x ∉ L, geen certificaat c bestaat zodat V(x, c) aanvaardt.
...
**Reductie**:
Een reductie is een manier om een probleem A om te zetten in een probleem B, zodat een oplossing voor B direct een oplossing voor A oplevert. In de context van NP wordt vaak gesproken van **polynomiale reductie** (notatie: A ≤p B), wat betekent:

- Er bestaat een functie f, berekenbaar in polynomiale tijd, die een instantie x van probleem A omzet in een instantie f(x) van probleem B, zodat x ∈ A ⇔ f(x) ∈ B.
...
- SAT = {⟨φ⟩ | φ is een propositionele formule in CNF die satisfiable is}.
- De stelling beweert dat SAT in NP zit en dat voor elke taal L ∈ NP, er een polynomiale reductie bestaat van L naar SAT.

**Reductie in het bewijs**:
Het bewijs van de stelling van Cook-Levin gebruikt een reductie van een willekeurige taal L ∈ NP naar SAT. De reductie werkt als volgt:

- Voor een taal L ∈ NP bestaat er een polynomiale verifiër V (een deterministische Turingmachine) die in tijd p(n) beslist of een input x ∈ L gegeven een certificaat c.
- De reductie construeert een propositionele formule φ in CNF die simuleert of V(x, c) aanvaardt voor een certificaat c.
- **Constructie**:
  - De formule φ beschrijft de configuraties van V op input ⟨x, c⟩ over p(n) stappen, inclusief de toestand van de tape, de positie van de kop, en de toestand van de machine.
  - Variabelen in φ vertegenwoordigen de inhoud van de tape, de toestand van V, en de positie van de kop op elk tijdstip t (0 ≤ t ≤ p(n)).
  - Clauses in φ zorgen ervoor dat:
    - De initiële configuratie overeenkomt met V startend op ⟨x, c⟩.
    - Elke stap van V voldoet aan de transitiefunctie van de Turingmachine.
    - Aan het einde van de berekening bevindt V zich in een accepterende toestand.
  - De grootte van φ is polynomiaal in p(n), omdat het aantal variabelen en clauses afhankelijk is van de grootte van de configuraties (O(p(n)²)).
- **Correctheid**:
  - Als x ∈ L, bestaat er een certificaat c zodat V(x, c) aanvaardt, en φ is satisfiable (er bestaat een toewijzing van variabelen die de berekening van V beschrijft).
  - Als x ∉ L, is er geen certificaat c zodat V(x, c) aanvaardt, en φ is niet satisfiable.
- Deze reductie is polynomiaal, omdat de constructie van φ in polynomiale tijd kan worden uitgevoerd.

De gebruikte reductie is dus een **polynomiale reductie van een willekeurige taal L ∈ NP naar SAT**.
...
- Er een polynomiale functie p bestaat, zodat V(x, c) in tijd O(p(|x|)) beslist of x ∈ L.
- Voor x ∈ L bestaat er een certificaat c (met |c| ≤ p(|x|)) zodat V(x, c) aanvaardt.
- Voor x ∉ L bestaat er geen certificaat c zodat V(x, c) aanvaardt.

**Beschrijving van SUBSETSUM**:
Het SUBSETSUM-probleem wordt gedefinieerd als:

- SUBSETSUM = {⟨S, t⟩ | S = {x₁, ..., xₖ} is een multiset van natuurlijke getallen, en er bestaat een deel-multiset T ⊆ S zodat ∑\_{y∈T} y = t}.
...
### b) (2pt) Beschrijf de reductie 3SAT ≤p SUBSETSUM in het algemeen én illustreer de reductie voor φ = (x₁ ∨ x₂ ∨ x₃) ∧ (¬x₂ ∨ x₃ ∨ x₄) ∧ (x₁ ∨ ¬x₂ ∨ ¬x₄). Toon aan in het algemeen dat een oplossing van het verkregen SUBSETSUM probleem overeenkomt met het satisfieerbaar zijn van het propositionele formule waarvan men vertrekt.

**Algemene beschrijving van de reductie 3SAT ≤p SUBSETSUM** (zie pagina 126-130):
De reductie zet een 3CNF-formule φ met ℓ variabelen x₁, ..., xₗ en k clauses c₁, ..., cₖ om in een SUBSETSUM-instantie ⟨S, t⟩, zodanig dat φ satisfiable is als en slechts als er een deel-multiset T ⊆ S bestaat met ∑\_{y∈T} y = t.
...
- **Polynomiale tijd**: De constructie van S en t vereist O(ℓk) operaties (voor het vullen van de tabellen), wat polynomiaal is in de grootte van φ.

**Illustratie voor φ = (x₁ ∨ x₂ ∨ x₃) ∧ (¬x₂ ∨ x₃ ∨ x₄) ∧ (x₁ ∨ ¬x₂ ∨ ¬x₄)**:
...
We tonen aan dat φ satisfiable is ⇔ ⟨S, t⟩ ∈ SUBSETSUM.

- **Voorwaartse richting (φ satisfiable ⇒ ⟨S, t⟩ ∈ SUBSETSUM)**:
  - Stel dat φ satisfiable is, met een toewijzing ν: {x₁, ..., xₗ} → {0, 1} die φ waar maakt.
  - Construeer T ⊆ S:
    ...
    - Voor de laatste k posities (clauses):
      - Omdat ν φ waar maakt, heeft elke clause cⱼ minstens één waar literal.
  ...
  - Dus, ⟨S, t⟩ ∈ SUBSETSUM.
- **Omgekeerde richting (⟨S, t⟩ ∈ SUBSETSUM ⇒ φ satisfiable)**:
  ...
  - Dus, ν maakt φ satisfiable.
- **Conclusie**: De reductie is correct, want φ is satisfiable ⇔ ⟨S, t⟩ ∈ SUBSETSUM.

**Voorbeeldverificatie** (optioneel, ter illustratie):
Voor φ = (x₁ ∨ x₂ ∨ x₃) ∧ (¬x₂ ∨ x₃ ∨ x₄) ∧ (x₁ ∨ ¬x₂ ∨ ¬x₄), een satisfiable toewijzing is ν(x₁) = 1, ν(x₂) = 0, ν(x₃) = 1, ν(x₄) = 0:

- T = {y₁, z₂, y₃, z₄, g₂, h₂, g₃, h₃}:
  - y₁ = 1000101, z₂ = 0100011, y₃ = 0010110, z₄ = 0001001.
  - g₂ = h₂ = 0000010, g₃ = h₃ = 0000001.
  - Som: 1111333 (controleer per positie).
- Dit komt overeen met een oplossing voor SUBSETSUM, en ν maakt φ waar.
